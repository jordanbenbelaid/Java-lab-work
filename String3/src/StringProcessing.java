...
// if a word matches exactly, it returns our message otherwise it returns the
	...
	public void FindCount(String message, String find) {
		...
		for (i = 0; i < message.length(); i++) {
			// comparing first letter of string to the first character of the word we are
			// finding
			if (message.substring(i, i + 1).equals(find.substring(0, 1))) {

				// counts the input word of choice amount of letters, and if matches the actual
				// word, it adds to the count
				// i + find.length() is here instead of i+1 as it counts the whole string we
				// want, not just the next letter
	...
	public void RemoveDuplicates(String message) {
		...
		for (i = 0; i < message.length(); i++) {
			if (message.substring(i, i + 1).equals(" ")) {
				if (CheckForDuplicates(word) == true) {
					newMessage += " " + word;
				}
				word = "";
			} else {
	...
	public Boolean CheckForDuplicates(String word) {
