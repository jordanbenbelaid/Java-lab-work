...
//	List<Person> findPersonByNameAndAge(String name, int age);
	...
	//native query
	@Query(value ="SELECT p from Person p WHERE age > 25", nativeQuery = true)
	...
	@Query("SELECT p from Person p WHERE age < 26")
	List<Person> findPersonByAgeLessThanJPQL(int age);

	@Query(value = "SELECT p from Person p WHERE age < 26", nativeQuery = true)
